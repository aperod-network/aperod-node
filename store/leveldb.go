...
var (
	...
	prefixAVM        = []byte("a/")  // a/<opaque AVM key>       → contract state/code/receipt
)
...
// AVMWrite is one opaque AVM state mutation. A nil Value deletes Key.
type AVMWrite struct {
	Key   []byte
	Value []byte
}

// GetAVMState reads one value from the isolated AVM key namespace.
func (d *DB) GetAVMState(key []byte) ([]byte, bool, error) {
	if len(key) == 0 {
		return nil, false, fmt.Errorf("store: empty AVM key")
	}
	fullKey := append(append([]byte{}, prefixAVM...), key...)
	value, err := d.get(fullKey)
	return value, value != nil, err
}

// ApplyAVMState commits a standalone AVM write set atomically. Canonical block
// acceptance must use CommitRawBlockWithAVM instead so state and tip cannot
// diverge across a crash.
func (d *DB) ApplyAVMState(writes []AVMWrite) error {
...
// CommitRawBlockWithAVM atomically fsyncs the block body, canonical height
// index, canonical tip, AVM write set and a per-height commitment marker.
func (d *DB) CommitRawBlockWithAVM(
	hash crypto.Hash32,
	height uint64,
	data []byte,
	writes []AVMWrite,
	...
	batch.Put(append(append([]byte{}, prefixMeta...), []byte("tip/hash")...), hash[:])
	batch.Put(append(append([]byte{}, prefixMeta...), []byte("tip/height")...), heightBytes[:])

	markerKey := append(append([]byte{}, prefixMeta...), []byte("avm/commit/")...)
	markerKey = append(markerKey, heightBytes[:]...)
	markerValue := make([]byte, 0, len(hash)+len(writeSetCommitment))
	markerValue = append(markerValue, hash[:]...)
	markerValue = append(markerValue, writeSetCommitment[:]...)
	batch.Put(markerKey, markerValue)
	return d.db.Write(batch, &opt.WriteOptions{Sync: true})
}

// GetAVMCommitment returns the canonical block hash and AVM write-set
// commitment recorded for a height.
func (d *DB) GetAVMCommitment(height uint64) (crypto.Hash32, crypto.Hash32, bool, error) {
	var heightBytes [8]byte
	binary.LittleEndian.PutUint64(heightBytes[:], height)
	key := append(append([]byte{}, prefixMeta...), []byte("avm/commit/")...)
	key = append(key, heightBytes[:]...)
	value, err := d.get(key)
	if err != nil || value == nil {
		return crypto.Hash32{}, crypto.Hash32{}, false, err
	}
	if len(value) != 64 {
		return crypto.Hash32{}, crypto.Hash32{}, false, fmt.Errorf("store: malformed AVM commitment at height %d", height)
	}
	var blockHash, commitment crypto.Hash32
	copy(blockHash[:], value[:32])
	copy(commitment[:], value[32:])
	return blockHash, commitment, true, nil
}

// IterAVMState visits every persisted AVM entry in key order. The slices passed
// to fn are copies and remain valid after fn returns.
func (d *DB) IterAVMState(fn func(key, value []byte) error) error {
	iter := d.db.NewIterator(util.BytesPrefix(prefixAVM), nil)
	defer iter.Release()
	for iter.Next() {
		key := append([]byte(nil), iter.Key()[len(prefixAVM):]...)
		value := append([]byte(nil), iter.Value()...)
...
// ReplaceAVMStateAndCommitment atomically replaces the complete AVM namespace
// and writes the canonical tip marker. It is used only by startup replay. A
// crash can therefore expose either the old state or the complete rebuilt
// state, never a partially rebuilt namespace.
func (d *DB) ReplaceAVMStateAndCommitment(
	writes []AVMWrite,
	height uint64,
	...
	iter.Release()
	if err := iter.Error(); err != nil {
		return fmt.Errorf("store: scan AVM state for replacement: %w", err)
	}
	for _, write := range writes {
		if len(write.Key) == 0 || write.Value == nil {
			return fmt.Errorf("store: invalid AVM replacement entry")
		}
		fullKey := append(append([]byte{}, prefixAVM...), write.Key...)
		batch.Put(fullKey, write.Value)
	}
	var heightBytes [8]byte
	binary.LittleEndian.PutUint64(heightBytes[:], height)
	markerKey := append(append([]byte{}, prefixMeta...), []byte("avm/commit/")...)
	markerKey = append(markerKey, heightBytes[:]...)
	markerValue := append(append([]byte{}, blockHash[:]...), writeSetCommitment[:]...)
	batch.Put(markerKey, markerValue)
	return d.db.Write(batch, &opt.WriteOptions{Sync: true})
}

// GetCanonicalHash returns the exact hash stored in the canonical height
// index. Malformed entries are reported as corruption rather than zero-padded.
func (d *DB) GetCanonicalHash(height uint64) (crypto.Hash32, bool, error) {
	value, err := d.get(heightKey(height))
	if err != nil || value == nil {
...
func avmBatch(writes []AVMWrite) (*leveldb.Batch, error) {
	batch := new(leveldb.Batch)
	for _, write := range writes {
		if len(write.Key) == 0 {
			return nil, fmt.Errorf("store: empty AVM key")
		}
		fullKey := append(append([]byte{}, prefixAVM...), write.Key...)
		if write.Value == nil {
