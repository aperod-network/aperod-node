...
func TestIndexer_IndexBlock_AddressMappingIdempotent(t *testing.T) {
	idx := testIndexer(t)
	b := sampleBlock(999_002_1)
	tx := sampleTx(b.Hash, b.Height, 0, false)
	mapping := explorer.AddrTxData{
		Address: "APRduplicate-mapping-9990021", TxHash: tx.Hash,
		BlockHeight: b.Height, TxIndex: 0, OutputIndex: 0,
	}
	if err := idx.IndexBlock(b, []explorer.TxData{tx}, []explorer.AddrTxData{mapping}); err != nil {
		t.Fatalf("first IndexBlock: %v", err)
	}
	if err := idx.IndexBlock(b, []explorer.TxData{tx}, []explorer.AddrTxData{mapping}); err != nil {
		t.Fatalf("second IndexBlock: %v", err)
	}
	rows, err := idx.GetAddrTxs(mapping.Address, 10, 0)
	if err != nil {
		t.Fatalf("GetAddrTxs: %v", err)
	}
	matches := 0
	for _, row := range rows {
		if row.TxHash == mapping.TxHash && row.BlockHeight == mapping.BlockHeight &&
			row.TxIndex == mapping.TxIndex && row.OutputIndex == mapping.OutputIndex {
			matches++
		}
	}
	if matches != 1 {
		t.Fatalf("duplicate address mapping count = %d, want 1", matches)
	}
}

func TestIndexer_IndexBlock_TotalTxsCountsOnlyNewTransactions(t *testing.T) {
	idx := testIndexer(t)
	before, err := idx.GetStats()
	if err != nil {
		t.Fatalf("GetStats before: %v", err)
	}
	b := sampleBlock(999_002_2)
	txs := []explorer.TxData{
		sampleTx(b.Hash, b.Height, 0, false),
		sampleTx(b.Hash, b.Height, 1, false),
	}
	// The integration database is intentionally persistent across test runs.
	// Give this assertion fresh transaction keys so an earlier run cannot make
	// the first insert look like an idempotent replay.
	runSuffix := fmt.Sprintf("-%d", time.Now().UnixNano())
	txs[0].Hash += runSuffix
	txs[1].Hash += runSuffix
	if err := idx.IndexBlock(b, txs, nil); err != nil {
		t.Fatalf("first IndexBlock: %v", err)
	}
	if err := idx.IndexBlock(b, txs, nil); err != nil {
	...
	after, err := idx.GetStats()
	if err != nil {
		t.Fatalf("GetStats after: %v", err)
	}
	if got, want := after.TotalTxs-before.TotalTxs, len(txs); got != want {
		t.Fatalf("total_txs increment = %d, want %d (only newly inserted rows)", got, want)
	}
}
