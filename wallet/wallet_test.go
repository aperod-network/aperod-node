...
func TestWordlistIsCanonicalBIP39English(t *testing.T) {
	if err := validateBIP39WordList(); err != nil {
		t.Fatalf("canonical BIP-39 English wordlist validation failed: %v", err)
	}
}
...
func TestEntropyToMnemonicOfficialBIP39Vectors(t *testing.T) {
	tests := []struct {
		entropy  string
		mnemonic string
	}{
	...
	for _, tt := range tests {
		...
		if err != nil {
			t.Fatalf("EntropyToMnemonic(%s): %v", tt.entropy, err)
		}
		if got != tt.mnemonic {
			t.Errorf("EntropyToMnemonic(%s):\nwant %q\n got %q", tt.entropy, tt.mnemonic, got)
		}
	}
}
...
func TestValidateMnemonicRejectsNonCanonicalOldPhrase(t *testing.T) {
	// This was emitted by the pre-APD-2026-007 truncated-list implementation
	// for entropy 20000000000000000000000000000000. There is intentionally no
	// legacy index remapping: it is rejected under the canonical BIP-39 list.
	oldPhrase := "camera abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon ability"
	if err := ValidateMnemonic(oldPhrase); err == nil {
		t.Fatal("non-canonical phrase unexpectedly accepted")
	} else if !strings.Contains(err.Error(), "checksum mismatch") {
		t.Fatalf("non-canonical phrase failed unexpectedly: %v", err)
	}
}

func TestLegacyMnemonicRecoveryCompatibility(t *testing.T) {
	oldPhrase := "camera abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon ability"
	legacy, err := ValidateMnemonicForRecovery(oldPhrase)
	if err != nil {
		t.Fatalf("legacy mnemonic rejected for recovery: %v", err)
	}
	if !legacy {
		t.Fatal("legacy mnemonic was not identified as legacy")
	}
	if _, err := DeriveFromMnemonic(oldPhrase, "", 0, 0); err != nil {
		t.Fatalf("legacy mnemonic derivation failed: %v", err)
	}
}
