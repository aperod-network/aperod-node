...
// scrypt parameters (N=2^17, r=8, p=1 — OWASP recommended for interactive login)
const (
	scryptN = 1 << 17 // 131072
	scryptR = 8
	scryptP = 1
...
func EncryptMnemonic(mnemonic, password, address string) (*KeystoreV1, error) {
	...
	aesKey, err := scrypt.Key([]byte(password), salt, scryptN, scryptR, scryptP, keyLen)
	if err != nil {
...
func DecryptMnemonic(ks *KeystoreV1, password string) (string, error) {
	...
	aesKey, err := scrypt.Key([]byte(password), salt, kp.N, kp.R, kp.P, keyLen)
	if err != nil {
