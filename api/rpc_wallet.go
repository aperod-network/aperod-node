...
// repairPhantomKeyImages cross-checks spent-index conflicts against confirmed
// canonical blocks. A key image is cleared only when every block from the
// source UTXO's creation height through the current tip was readable and none
// of those blocks contains it. Missing/pruned block data fails closed.
func (s *Server) repairPhantomKeyImages(result *core.BuildResult) (bool, error) {
	if result == nil || s.chain == nil || s.utxos == nil {
		return false, nil
	}
	repaired := false
	tip := s.chain.Height()
	for i, inp := range result.Tx.Inputs {
		if !s.utxos.IsSpent(inp.KeyImage) {
			continue
		}
		if i >= len(result.SelectedUTXOs) {
			return repaired, fmt.Errorf("cannot verify spent key image at input %d: source UTXO is unknown", i)
		}
		start := result.SelectedUTXOs[i].BlockHeight
		found, err := s.confirmedChainHasKeyImage(inp.KeyImage, start, tip)
		if err != nil {
			return repaired, fmt.Errorf("cannot verify spent key image for utxo %x[%d]: %w",
...
continue
		...
		s.log.Warn("WALLET_SEND: cleared phantom spent key image after confirmed-chain verification",
			...
			"out_idx", result.SelectedUTXOs[i].OutputIndex,
			"key_image", fmt.Sprintf("%x", inp.KeyImage[:]))
	}
	return repaired, nil
}

func (s *Server) confirmedChainHasKeyImage(want crypto.KeyImage, start, tip uint64) (bool, error) {
	wantCanonical, err := crypto.CanonicalKeyImage(want)
	if err != nil {
		return false, fmt.Errorf("invalid key image: %w", err)
	}
	for height := start; height <= tip; height++ {
		block := s.chain.GetByHeight(height)
		if block == nil && s.blockStore != nil {
			raw, readErr := s.blockStore.GetRawBlockByHeight(height)
			if readErr != nil {
				return false, fmt.Errorf("read block %d: %w", height, readErr)
			}
			if raw != nil {
				var decoded core.Block
				if err := json.Unmarshal(raw, &decoded); err != nil {
					return false, fmt.Errorf("decode block %d: %w", height, err)
				}
				block = &decoded
			}
		}
		if block == nil {
			return false, fmt.Errorf("block %d unavailable", height)
		}
		for _, tx := range block.Txs {
			for _, txIn := range tx.Inputs {
				canonical, err := crypto.CanonicalKeyImage(txIn.KeyImage)
