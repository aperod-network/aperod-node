...
type MempoolConfig struct {
	...
	MinFeeRate uint64        // minimum fee per byte in base units
}
...
func (m *Mempool) Add(tx Transaction) error {
	...
	if feeRate < m.cfg.MinFeeRate {
		return fmt.Errorf("mempool: fee rate too low: %d < %d nAPR/byte", feeRate, m.cfg.MinFeeRate)
	}
