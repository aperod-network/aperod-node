...
// emptyMerkleRoot is the canonical Merkle root for a block that contains no
// transactions.  It is defined as SHA3-256("aperod/merkle-empty/v1") so that
// it is deterministic, non-zero, and cannot be confused with an uninitialised
// hash field.
var emptyMerkleRoot = crypto.HashBytes([]byte("aperod/merkle-empty/v1"))

// MerkleRoot computes the Merkle root of a list of transactions.
// Uses SHA3-256 pairwise. If the list is empty, returns emptyMerkleRoot (a
// well-known non-zero constant). If the list is odd, the last element is
// duplicated.
