...
type Config struct {
	...
	// BroadcastRetryDelay is how long BroadcastBlock waits before its one
	// bounded retry after a failed peer.Send (temporary write timeout,
	// congested socket).  Without the retry a missed peer would only
	// recover through the keepalive Pong cycle (10 s) or the GetBlock
	// stall timer (15 s), delaying block propagation.  Exactly one retry
	// is attempted per peer per broadcast; the retry is skipped when the
	// peer has already been disconnected/evicted.  Default: 200ms.
...
type Peer struct {
	...
	// poisoned is set (under mu, atomically with the failing write) when a
	// send left a partial frame on the wire.  Once poisoned the stream
	// framing is permanently desynchronized: every further send is
	// rejected and the connection has already been closed.  Guarded by mu.
	poisoned bool
}

// Send transmits a message to this peer.
func (p *Peer) Send(msgType MessageType, payload interface{}) error {
	_, err := p.sendN(msgType, payload)
	return err
}

// sendN is Send with byte accounting: it additionally reports how many bytes
// were written to the connection.  A failed send with wrote == 0 left the
// stream framing intact (safe to retry on the same connection); a failed
// send with wrote > 0 left a partial frame on the wire, after which the only
// safe recovery is closing the connection.
// send with wrote > 0 left a partial frame on the wire; in that case the
// stream is marked poisoned and the connection is closed atomically with the
// failing write (still holding p.mu), so no concurrent Send can append a new
// frame to the unfinished one before teardown.  All subsequent sends on a
// poisoned peer are rejected with net.ErrClosed.
func (p *Peer) sendN(msgType MessageType, payload interface{}) (wrote int, err error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.poisoned {
	...
	wrote, err = writeMsgN(p.conn, msgType, payload)
	if err != nil && wrote > 0 {
		// Partial frame on the wire: poison and close BEFORE releasing
		// p.mu so no interleaving sender can write into the broken stream.
		p.poisoned = true
		p.conn.Close()
	}
	return wrote, err
...
// duplicateIdentityMaxEvents caps the in-memory ring buffer for
// duplicate-identity fingerprint conflict events.
const duplicateIdentityMaxEvents = 50

// DuplicateIdentityEvent records one identity-conflict rejection: a peer
// presented the same TLS SPKI fingerprint as this node's own identity key
// (typically because p2p_identity.key was copied along with rsynced chain
// data).  Stored in a ring buffer and exposed via GetDuplicateIdentityEvents
// so the API server can poll and surface them in the Admin Panel
// notification log.
type DuplicateIdentityEvent struct {
	// Addr is the remote "IP:port" address of the rejected peer.
	Addr string `json:"addr"`
	// Fingerprint is the shared hex-encoded SHA-256 SPKI fingerprint.
...
type Host struct {
	...
	// sendHook, when non-nil, replaces Peer.Send for all outbound
	// broadcast sends.  Used exclusively by unit tests to inject
	// transient send failures; nil in production (sendToPeer falls back
	// to Peer.Send).  Atomic so tests can install it without racing the
	// broadcast path.
	sendHook atomic.Pointer[func(*Peer, MessageType, interface{}) (int, error)]
	...
	// duplicateIdentityMu guards duplicateIdentityEvents so handleConn can
	// append events concurrently with the API server reading them.
	duplicateIdentityMu sync.Mutex
	// duplicateIdentityEvents is a ring buffer of identity-conflict events
	// for the Admin Panel notification log.
	// Capped at duplicateIdentityMaxEvents.
	duplicateIdentityEvents []DuplicateIdentityEvent
...
func NewHost(cfg Config, handler Handler, log *slog.Logger) *Host {
	...
	if cfg.BroadcastRetryDelay == 0 {
		cfg.BroadcastRetryDelay = 200 * time.Millisecond
	}
...
// recordDuplicateIdentityEvent appends one identity-conflict rejection to the
// ring buffer read by GetDuplicateIdentityEvents.  Called from handleConn just
// before the conflicting connection is closed.
func (h *Host) recordDuplicateIdentityEvent(addr, fingerprint string) {
	h.duplicateIdentityMu.Lock()
	defer h.duplicateIdentityMu.Unlock()
	h.duplicateIdentityEvents = append(h.duplicateIdentityEvents, DuplicateIdentityEvent{
		Addr:        addr,
		Fingerprint: fingerprint,
		At:          time.Now(),
	})
	if len(h.duplicateIdentityEvents) > duplicateIdentityMaxEvents {
		h.duplicateIdentityEvents = h.duplicateIdentityEvents[len(h.duplicateIdentityEvents)-duplicateIdentityMaxEvents:]
	}
}

// GetDuplicateIdentityEvents returns all duplicate-identity conflict events
// that occurred at or after since.  Thread-safe; safe to call concurrently
// with handleConn.  Returns an empty (non-nil) slice when no events match.
func (h *Host) GetDuplicateIdentityEvents(since time.Time) []DuplicateIdentityEvent {
	h.duplicateIdentityMu.Lock()
	defer h.duplicateIdentityMu.Unlock()
	out := make([]DuplicateIdentityEvent, 0)
	for _, e := range h.duplicateIdentityEvents {
		if !e.At.Before(since) {
			out = append(out, e)
		}
	}
...
// PeerListEntry is a snapshot of one connected peer's key state.
// Returned by GetPeerList for the /api/v1/network/peers REST endpoint.
...
// GetPeerList returns a snapshot of all currently connected peers with their
// last-reported chain heights and connection direction.  Heights are updated
// on every MsgPong reply; a height of 0 means the peer connected but has not
// yet responded to a keepalive Ping.
func (h *Host) GetPeerList() []PeerListEntry {
	h.mu.RLock()
	defer h.mu.RUnlock()
	out := make([]PeerListEntry, 0, len(h.peers))
	for addr, p := range h.peers {
		dir := "in"
		if p.outbound {
			dir = "out"
		}
		out = append(out, PeerListEntry{Addr: addr, Height: p.height, Direction: dir})
	}
	return out
}
...
//
// When a peer.Send fails with a transient error (temporary write timeout,
// congested socket), exactly one bounded retry is scheduled after
// cfg.BroadcastRetryDelay so the peer receives the block quickly instead of
// waiting for the keepalive Pong cycle (10 s) or the GetBlock stall timer
// (15 s) to recover.  The retry is skipped when the peer has already been
// disconnected or evicted from the peer table, so no retry storms occur.
func (h *Host) BroadcastBlock(block *core.Block) {
	...
	sb := blockToMsg(block)
	for _, p := range peers {
		wrote, err := h.sendToPeer(p, MsgBlock, sb)
		if err == nil {
			continue
		}
		if wrote > 0 {
			// Partial frame reached the wire: the stream framing is now
			// desynchronized and ANY further send (retry or otherwise)
			// would corrupt the receiver's parser.  sendN already
			// poisoned the peer and closed the connection atomically
			// under the peer write lock; conn.Close() here is a
			// defensive no-op for the test-hook path.  The normal
			// disconnect/reconnect lifecycle takes over.
			h.log.Warn("broadcast block partial write; connection closed (no retry)",
				"peer", p.addr, "wrote", wrote, "err", err)
			p.conn.Close()
			continue
		}
		if !isRetryableSendErr(err) {
			// Permanent failure (closed connection, marshal error,
			// oversize message): a retry cannot succeed, so rely on the
			// normal disconnect / keepalive lifecycle instead.
			h.log.Warn("broadcast block failed (permanent, no retry)",
				"peer", p.addr, "err", err)
			continue
		}
		// Zero bytes written + transient error: framing intact, one short
		// retry is safe and removes the 10–15 s fallback gap.
		h.log.Warn("broadcast block failed, scheduling one retry",
			"peer", p.addr, "err", err,
			"retry_in", h.cfg.BroadcastRetryDelay)
		go h.retryBlockSend(p, sb)
	}
}

// isRetryableSendErr reports whether a Peer.Send failure is transient enough
// that a single short retry could plausibly succeed.  Retryable: write
// timeouts (congested socket) and kernel buffer exhaustion.  Not retryable:
// closed/reset connections, broken pipes, marshal errors, oversize messages —
// for those the peer is (being) torn down or the payload itself is bad, so a
// retry would only waste a delayed write.
...
// sendToPeer transmits one message to a peer, routing through the test hook
// when one is installed (unit tests use it to inject transient send failures).
func (h *Host) sendToPeer(p *Peer, msgType MessageType, payload interface{}) (int, error) {
	if hook := h.sendHook.Load(); hook != nil {
		return (*hook)(p, msgType, payload)
	}
	return p.sendN(msgType, payload)
}

// retryBlockSend performs the single bounded retry for a failed
// BroadcastBlock send.  It waits cfg.BroadcastRetryDelay, verifies the peer
// is still registered in the peer table (same *Peer instance — a reconnect
// under the same addr creates a new instance and must not be retried with
// stale state), and attempts exactly one more Send.  A second failure is
// logged and left to the existing keepalive / stall-timer fallbacks.
func (h *Host) retryBlockSend(p *Peer, sb interface{}) {
	select {
	case <-h.done:
		return
	case <-time.After(h.cfg.BroadcastRetryDelay):
	}
	// Skip the retry when the connection is already closed/evicted.
	// The registration check and the send happen under the same read
	// lock so eviction (DropPeer/BanPeer take h.mu.Lock) cannot
	// interleave between the check and the send: the retry either
	// completes before the peer is removed or is skipped entirely.
	// Holding RLock during the send is acceptable because retries are
	// rare (one per failed broadcast per peer) and bounded by the 10 s
	// write deadline in Peer.Send.
	h.mu.RLock()
	defer h.mu.RUnlock()
	cur, ok := h.peers[p.addr]
	if !ok || cur != p {
		return
	}
	wrote, err := h.sendToPeer(p, MsgBlock, sb)
	...
	if wrote > 0 {
		// Partial frame on the retry: stream is desynchronized.  sendN
		// already poisoned the peer and closed the connection under the
		// peer write lock; Close here is a defensive no-op for the
		// test-hook path.  (conn.Close is safe under h.mu.RLock — it
		// takes no host locks.)
		h.log.Warn("broadcast block retry partial write; connection closed",
			"peer", p.addr, "wrote", wrote, "err", err)
		p.conn.Close()
		return
	}
	h.log.Warn("broadcast block retry failed; relying on keepalive/stall fallback",
		"peer", p.addr, "err", err)
}
...
// ReconnectBackoffActive reports whether at least one configured bootnode is
// currently inside its exponential dial back-off window (i.e. the host has
// recorded consecutive dial failures and is deliberately waiting before the
// next attempt).  Combined with PeerCount()==0 this is the exact "relay is
// silently stuck retrying its only known peer" state: the node looks alive
// but will not sync until the back-off timer expires and the peer comes back.
// Exposed via the REST API so external monitoring can alert on it.
func (h *Host) ReconnectBackoffActive() bool {
	now := time.Now()
	h.bootnodeMu.Lock()
	defer h.bootnodeMu.Unlock()
	for _, e := range h.bootnodeFailState {
		if !e.nextDial.IsZero() && now.Before(e.nextDial) {
			return true
		}
	}
	return false
...
func (h *Host) handleConn(conn net.Conn, outbound bool) {
	...
	if tlsConn, ok := conn.(*tls.Conn); ok {
		...
		if h.cfg.SelfFingerprint != "" && fp == h.cfg.SelfFingerprint {
			...
			h.recordDuplicateIdentityEvent(addr, fp)
			conn.Close()
			return
		}
