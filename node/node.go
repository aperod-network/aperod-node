...
func New(cfg *Config, log *slog.Logger) (*Node, error) {
	...
	if genesis := chain.Genesis(); genesis == nil {
		return nil, fmt.Errorf("guardian fund startup validation requires canonical genesis")
	} else if err := db.BindGuardianFundConfig(0, genesis.Hash()); err != nil {
		// This legacy node constructor has no Guardian activation fields. It
		// must not open a database already bound to an activated configuration
		// and silently run with the feature disabled.
		return nil, err
	}
