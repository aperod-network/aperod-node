...
func run() error {
	...
	//
	// Threshold: 85 % of GOMEMLIMIT.  The previous 65 % threshold was
	// below the relay's normal sync RSS (1.95 GB with a 3 GB limit), so
	// catch-up repeatedly triggered graceful restarts before making
	// durable progress. Fifteen percent still leaves bounded headroom
	// while allowing the configured Go memory limit to do its job.
	{
		limit := debug.SetMemoryLimit(-1) // -1 = query without changing
		if limit > 0 {
			threshold := uint64(limit) * 85 / 100
			go func() {
	...
	if os.Getenv("SCHED_RESTART_ENABLED") == "true" && cfg.Maintenance.RestartAt != "" {
