...
var validatorProveSessionCmd = &cobra.Command{
	Use:   "prove-session",
	Short: "Sign a website validator-session challenge locally",
	RunE: func(cmd *cobra.Command, args []string) error {
		challengePath, _ := cmd.Flags().GetString("challenge")
		keyPath, _ := cmd.Flags().GetString("key-file")
		outPath, _ := cmd.Flags().GetString("out")
		if challengePath == "" || keyPath == "" || outPath == "" {
			return fmt.Errorf("--challenge, --key-file and --out are required")
		}
		body, err := os.ReadFile(challengePath)
		if err != nil {
		...
		hashRaw, err := hex.DecodeString(challenge.MessageHash)
		if err != nil || len(hashRaw) != 32 {
			return fmt.Errorf("challenge message_hash must be 32-byte hex")
		}
		canonical := strings.Join([]string{"aperod/validator-session/v1", challenge.ChallengeID, challenge.SessionHash,
			challenge.IdentityHash, challenge.Address, challenge.PubKey, challenge.ChainAnchor, challenge.ExpiresAt}, "\n")
		computed := crypto.HashBytes([]byte(canonical))
		if !bytes.Equal(computed[:], hashRaw) {
			return fmt.Errorf("challenge fields do not match message_hash")
		}
		keyRaw, err := os.ReadFile(keyPath)
		if err != nil {
			return fmt.Errorf("read validator key: %w", err)
		}
		trimmed := strings.TrimSpace(string(keyRaw))
		decoded, decErr := hex.DecodeString(trimmed)
		if decErr == nil {
			keyRaw = decoded
		}
		priv, err := crypto.ValidatorPrivKeyFromBytes(keyRaw)
		if err != nil {
			return fmt.Errorf("parse validator key: %w", err)
		}
		if priv.Public().Hex() != challenge.PubKey {
			return fmt.Errorf("validator key does not match challenge public key")
		}
		...
		response, err := json.MarshalIndent(map[string]string{"pub_key": challenge.PubKey, "signature": hex.EncodeToString(sig)}, "", "  ")
		if err != nil {
			return err
		}
		if err = os.WriteFile(outPath, response, 0600); err != nil {
			return fmt.Errorf("write response: %w", err)
		}
		fmt.Printf("Verified challenge expiry: %s\nWrote validator possession proof to %s\n", challenge.ExpiresAt, outPath)
		return nil
	},
}

var validatorApproveLPoDExitCmd = &cobra.Command{
	Use:   "approve-lpod-exit",
	Short: "Review and locally sign a chain-bound LPoD validator exit",
	RunE: func(cmd *cobra.Command, args []string) error {
		challengePath, _ := cmd.Flags().GetString("challenge")
		keyPath, _ := cmd.Flags().GetString("key-file")
		outPath, _ := cmd.Flags().GetString("out")
		...
		var c struct {
			...
			Action, PubKey, Amount, CurrentStake, Genesis, Nonce, Generation, StakeRef, Expiry string
			ExpiresAt                                                                          string `json:"expires_at"`
		}
		...
		get := func(name string) *string {
			var s string
			if raw := wire[name]; len(raw) > 0 && json.Unmarshal(raw, &s) == nil {
				return &s
			}
			return nil
		}
		for _, name := range []string{"action", "pub_key", "amount_napro", "current_stake_napro", "chain_anchor", "nonce", "generation", "stake_reference", "expiry_height", "expires_at"} {
			if get(name) == nil {
				return fmt.Errorf("challenge missing %s", name)
			}
		}
		c.Action = *get("action")
		c.PubKey = *get("pub_key")
		c.Amount = *get("amount_napro")
		c.CurrentStake = *get("current_stake_napro")
		c.Genesis = *get("chain_anchor")
		c.Nonce = *get("nonce")
		c.Generation = *get("generation")
		c.StakeRef = *get("stake_reference")
		c.Expiry = *get("expiry_height")
		c.ExpiresAt = *get("expires_at")
		parseU := func(name, value string) (uint64, error) {
			n, e := strconv.ParseUint(value, 10, 64)
			if e != nil {
				return 0, fmt.Errorf("%s is not uint64", name)
			}
			return n, nil
		}
		amount, err := parseU("amount_napro", c.Amount)
		if err != nil {
			return err
		}
		nonce, err := parseU("nonce", c.Nonce)
		if err != nil {
			return err
		}
		generation, err := parseU("generation", c.Generation)
		if err != nil {
			return err
		}
		expiry, err := parseU("expiry_height", c.Expiry)
		if err != nil {
			return err
		}
		action := core.StakeWithdraw
		if c.Action == "partial" {
			action = core.StakePartialWithdraw
		} else if c.Action != "full" {
			return fmt.Errorf("invalid exit action")
		}
		if action == core.StakeWithdraw && amount != 0 {
			return fmt.Errorf("full exit amount must be zero")
		}
		pubRaw, err := hex.DecodeString(c.PubKey)
		if err != nil || len(pubRaw) != 32 {
			return fmt.Errorf("invalid validator pubkey")
		}
		pub := crypto.ValidatorPubKey(pubRaw)
		genRaw, err := hex.DecodeString(c.Genesis)
		if err != nil || len(genRaw) != 32 {
			return fmt.Errorf("invalid chain anchor")
		}
		var genesis crypto.Hash32
		copy(genesis[:], genRaw)
		refRaw, err := hex.DecodeString(c.StakeRef)
		if err != nil || len(refRaw) != 32 {
			return fmt.Errorf("invalid stake reference")
		}
		var ref crypto.Hash32
		copy(ref[:], refRaw)
		keyRaw, err := os.ReadFile(keyPath)
		if err != nil {
			return fmt.Errorf("read validator key: %w", err)
		}
		if decoded, e := hex.DecodeString(strings.TrimSpace(string(keyRaw))); e == nil {
			keyRaw = decoded
		}
		priv, err := crypto.ValidatorPrivKeyFromBytes(keyRaw)
		if err != nil {
		...
		if priv.Public().Hex() != c.PubKey {
			return fmt.Errorf("validator key does not match exit")
		}
		fmt.Printf("\nDANGEROUS VALIDATOR EXIT\n  action: %s\n  validator: %s\n  amount_napro: %s (0 means full)\n  current_stake_napro: %s\n  chain: %s\n  nonce/generation: %s / %s\n  expiry height: %s\n  rewards stop after confirmation; cooldown and Guardian migration/auto-return apply.\n\nType EXIT %s to sign: ",
			c.Action, c.PubKey, c.Amount, c.CurrentStake, c.Genesis, c.Nonce, c.Generation, c.Expiry, c.PubKey[len(c.PubKey)-8:])
		line, _ := bufio.NewReader(os.Stdin).ReadString('\n')
		if strings.TrimSpace(line) != "EXIT "+c.PubKey[len(c.PubKey)-8:] {
			return fmt.Errorf("operator cancelled")
		}
		auth := core.StakeWithdrawalAuthorizationV2{Action: action, PubKey: pub, Amount: amount, Genesis: genesis, Nonce: nonce, Generation: generation, ExpiryHeight: expiry, StakeRef: ref}
		sig, err := priv.Sign(core.StakeWithdrawalSignMsgV2(auth))
		if err != nil {
			return err
		}
		auth.Signature = sig
		...
		payload, err := json.MarshalIndent(map[string]interface{}{"tx": tx}, "", "  ")
		if err != nil {
			return err
		}
		if err = os.WriteFile(outPath, payload, 0600); err != nil {
			return fmt.Errorf("write signed transaction: %w", err)
		}
		fmt.Printf("Wrote locally signed v2 exit transaction to %s\n", outPath)
		return nil
	},
}
...
func init() {
	...
	validatorProveSessionCmd.Flags().String("challenge", "", "Downloaded website challenge JSON")
	validatorProveSessionCmd.Flags().String("key-file", "", "Local validator private-key file (raw or hex)")
	validatorProveSessionCmd.Flags().String("out", "", "Write uploadable proof JSON")
	validatorCmd.AddCommand(validatorProveSessionCmd)
	validatorApproveLPoDExitCmd.Flags().String("challenge", "", "Downloaded website exit challenge JSON")
	validatorApproveLPoDExitCmd.Flags().String("key-file", "", "Local validator private-key file (raw or hex)")
	validatorApproveLPoDExitCmd.Flags().String("out", "", "Write uploadable signed transaction JSON")
	validatorCmd.AddCommand(validatorApproveLPoDExitCmd)
